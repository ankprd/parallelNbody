...
void move_particle(int i, particle_t*p, double step) {

  p->x_pos += (p->x_vel)*step;
  p->y_pos += (p->y_vel)*step;
  double x_acc = tempForces[2 * i]/p->mass;
  double y_acc = tempForces[2 * i + 1]/p->mass;
  p->x_vel += x_acc*step;
  p->y_vel += y_acc*step;
...
void run_simulation() {
  ...
  while (t < T_FINAL && nparticles>0) {
    ...
    for(i=0; i<nparticles; i++) {
        move_particle(i, &particles[i], dt);
    }
    /*for(i=0; i<nparticles; i++) {
      particle_t*p = &particles[i];
...
int main(int argc, char**argv)
{
  ...
  tempForces = (double*) malloc(2 * sizeof(double));
