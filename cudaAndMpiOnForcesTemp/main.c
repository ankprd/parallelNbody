...
void run_simulation(int rank, int nbT) {
  ...
  while (t < T_FINAL && nparticles>0) {
    ...
    /*for(i=0; i<nparticles; i++) {
      particle_t*p = &particles[i];
      printf("in main.c apres cuda particle={pos=(%f,%f), vel=(%f,%f), f = {%f, %f}}\n", p->x_pos, p->y_pos, p->x_vel, p->y_vel, p->x_force, p->y_force);
    }*/
    ...
    /*for(i=0; i<nparticles; i++) {
      particle_t*p = &particles[i];
      printf("in main.c after move particle={pos=(%f,%f), vel=(%f,%f), f = {%f, %f}}\n", p->x_pos, p->y_pos, p->x_vel, p->y_vel, p->x_force, p->y_force);
    }*/
...
int main(int argc, char**argv)
{
  ...
  if(rank == 0){
      ...
      FILE* f_out9 = fopen("temps.log", "w");

      fprintf(f_out9, "-----------------------------\n");
      fprintf(f_out9, "nparticles: %d\n", nparticles);
      fprintf(f_out9, "T_FINAL: %f\n", T_FINAL);
      fprintf(f_out9, "-----------------------------\n");
      fprintf(f_out9, "Simulation took %lf s to complete\n", duration);
